...
- 每张指导书默认使用 3 个工图位，并允许按工序在 1–6 格之间调整；所有版式均按从左到右、从上到下排列；
- 图片默认空白，等待人工上传和确认；
...
-> 从路线步骤派生纵向流程图 PNG
-> 每个路线步骤建立一张横向详细工序指导页
-> 填写动作、完整作业方法、材料输入、设备治具、工艺参数、品质、安全、异常和记录
-> 图片区保持待人工上传
-> 应用 DRAFT、空白签核与受控日期
-> 校验 DOCX
-> Word 转 PDF/PNG
...
上传、提交或发 PR 前必须完整执行
`DOCX_PREVIEW_UPLOAD_CHECKLIST.md`。该清单记录了分页脚本漏提交、Windows 预览目录被 Edge
占用以及 8787 端口同时运行新旧服务造成的预览故障，不能跳过。
...
- 每页指导书默认 3 格，允许各工序独立选择 1–6 格，均按自然顺序排列；
- 人工填写的 IE 项目压缩显示在右侧参数区；没有人工数据时只显示“未提供人工 IE 数据”，不得补造数值；
- 页面右上角“编辑当前页”继续支持局部文字修改、撤销、图片上传确认、图片格数和 IE 项目编辑；
- 每页批准、审核、制作值栏为空；
- 无空白夹页、截断、重叠、乱码或表格越界；
- 实际渲染页数等于 `1 + 工序数`。
